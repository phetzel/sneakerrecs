...
const QuestionsList = ({ results, history }) => {
  ...
  const colorSecondaryLink = colorSecondary
    ...
    : "Secondary Colors";
  const priceLink = price ? `Price: Under $${price.value}` : "Price: Any";
  ...
  return (
    <div className={`questions-list ${toggleClass}`} onClick={handleResults}>
      <ul>
        {maxQuestion > 1 && <li onClick={() => setQuestion(1)}>{styleLink}</li>}
        {maxQuestion > 2 && (
        ...
        {maxQuestion > 4 && <li onClick={() => setQuestion(4)}>{priceLink}</li>}
      </ul>

      {results && <h6 onClick={() => setQuestion(1)}>Redo questions</h6>}
    </div>
  );
};
